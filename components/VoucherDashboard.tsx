...
interface VoucherDashboardProps {
  history: VoucherData[];
  onLoad: (voucher: VoucherData) => void;
  onDelete: (voucherNumber: string) => void;
  onDownload: (voucher: VoucherData) => void;
}

export const VoucherDashboard: React.FC<VoucherDashboardProps> = ({ history, onLoad, onDelete, onDownload }) => {
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-12 text-slate-400">
        ...
        <p className="text-xs text-center mt-2 max-w-[200px] leading-relaxed text-slate-400">
            No generated vouchers found. Your document history will appear here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid gap-3">
        {sortedHistory.map((voucher) => {
          const totalAmount = voucher.grossAmount;
          const netAmount = totalAmount - (totalAmount * (voucher.tdsRate / 100));
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="space-y-1">
                    ...
                    <h3 className="font-bold text-slate-800 text-sm leading-tight group-hover:text-slate-900 transition-colors">
                        {voucher.payeeName || 'Unnamed Payee'}
                    </h3>
                </div>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <div className="flex gap-1">
                    ...
                    <button 
                        onClick={(e) => {
                            e.stopPropagation();
                            onDelete(voucher.voucherNumber);
                        }}
